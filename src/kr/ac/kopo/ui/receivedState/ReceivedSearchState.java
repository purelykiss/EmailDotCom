...
public class ReceivedSearchState extends BaseMailState {

	public ReceivedSearchState(IMailUI parent) {
	...
	public void execute() {
		...
		switch(inputNum) {
		case 1->{
				searchList = EmailService.getInstance().searchEmail(1, 1, searchPlusList, searchMinusList);
			}
		case 2->{
				searchList = EmailService.getInstance().searchEmail(1, 2, searchPlusList, searchMinusList);
			}
		case 3 -> {
				Set<Integer> tmp = new HashSet<Integer>();
				tmp.add(1);
				tmp.add(2);
				searchList = EmailService.getInstance().searchEmail(1, tmp, searchPlusList, searchMinusList);
			}
		case 4->{
				Set<Integer> tmp = new HashSet<Integer>();
				tmp.add(3);
						...
						if(input.equals("")) {
							...
							try {
								...
								if(n >= (curPage-1)*emailPerPage + 1 & n <= curPage*emailPerPage) {
									ViewManager.getInstance().ViewMail(1, searchList.get(n - 1 - (curPage - 1) * emailPerPage).getCode());
									flagInt = true;
								}
							} catch (Exception e) {}
							if(flagInt) {
